	...
	for _, layer in ipairs(Interfaces.UpgradeScroll:GetChildren()) do
		if layer and layer.Parent and layer:IsA("GuiObject") then
			...
			table.insert(UpgradeInfos[key], {
				Name = key,
				Button = buyButton,
				MaxFrame = frame:FindFirstChild("Max")
			})
		end
	end
...
local function GetPlot()
	...
	if not plot then
		local plots = workspace:FindFirstChild("PlayerEar")
	...
	Callback = function(value)
		...
		task.spawn(function()
			while Enableds.Stage do
				local stage = ProfileData.Stage
				local stageFolder = StageFolder:FindFirstChild("关卡"..tostring(stage))
				...
				if stageFolder then
					...
					while Enableds.Stage and checkPart and checkPart.CanCollide and stage < ProfileData.Checkpoint do
						SuperPivoTo(Character, surfacePart, rootPart, humanoid.HipHeight)
						task.wait()
					end

					local lastStage = ProfileData.Stage - 1
						...
						if WorldFishFolder then
							for _, child in ipairs(WorldFishFolder:GetChildren()) do
								...
								if child and child.Parent and child:IsA("Model") then
									...
									if stageId == nil or stageId ~= lastStage then continue end
...
Window:AddToggle({
	...
	Callback = function(value)
		...
		if not Packets.Upgrade then
			local ok, result = pcall(function()
				return ReplicatedStorage.Remote.Event.Upgrade["[C-S]BuyCashUpgrade"]
			end)
			if ok and result then Packets.Upgrade = result end
		end
		task.spawn(function()
				...
				for key, list in pairs(UpgradeInfos) do
					...
					if active or UpgradeActives.AllEnabled then
						for _, info in ipairs(list) do
							if not Enableds.Upgrade then break end
							if info.MaxFrame and info.MaxFrame.Visible == true then continue end
							if Packets.Upgrade then
								Packets.Upgrade:FireServer(key)
							else
