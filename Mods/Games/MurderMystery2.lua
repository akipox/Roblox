...
local Observe = loadstring(game:HttpGet("https://raw.githubusercontent.com/Crokier/Roblox/main/Packages/Observe/init.luau"))()
...
local BillboardPool = {}
local ActiveESP = {}
local CharacterActives={}

local CleanPlayer = Observe.ObservePlayer(function(player)
    local cleanCharacter = Observe.ObserveCharacter(player, function(character)
        CharacterActives[character]=true
        return function()
           CharacterActives[character]=nil
        end
    end)
    return function()
       cleanCharacter()
...
local function AddESP(child)
  ...
  local attempt, knife = 0, nil
  repeat
      for i,v in ipairs(child:GetChildren()) do
         if v and v.Parent and v:IsA("Tool") and v.Name=='Knife' then
            knife=v
            break
         end
      end
      attempt+= 1
      task.wait(1)
  until knife ~= nil and attempt>=10 or not child.Parent
	...
	childConnections.AncestryChanged = child.AncestryChanged:Connect(function(_, parent)
		if not parent or not child:IsDescendantOf(workspace) and childConnections.AncestryChanged.Connected then
			DestroyESP(child)
		end
	end)

  childConnections.KnifeAncestryChanged = knife.AncestryChanged:Connect(function(_, parent)
		if not parent or not child:IsDescendantOf(child) then
      childConnections.AncestryChanged:Disconnect()
			DestroyESP(child)
		end
	end)
...
local function ESPChameleon()
	...
	task.spawn(function()
		while Enableds.Chameleon do
			for character, enabled in pairs(CharacterActives) do
				if not Enableds.Chameleon then break end
				ProcessChameleon(character)
				task.wait()
			end
			task.wait(1)
		end
